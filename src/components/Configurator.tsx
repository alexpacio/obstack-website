import { useEffect, useState } from 'react';
import { BASELINE, DEFAULT_CONFIG, configFromSearch, configQuery, formatUsd, quote, type Config } from '../data/plans';
import { mailto, url } from '../lib/url';
import ConfigFields, { QuoteSummary, quoteText } from './ConfigFields';
import './checkout.css';

export default function Configurator() {
  const [config, setConfig] = useState<Config>(DEFAULT_CONFIG);
  const q = quote(config);
  const baseline = q.nodes * BASELINE.perHost;

  // Preselect from ?plan= (set by the plan cards).
  useEffect(() => {
    if (window.location.search) setConfig(configFromSearch(window.location.search));
  }, []);

  return (
    <div className="co co-embed">
      <div className="co-main">
        <ConfigFields config={config} onChange={setConfig} />
      </div>

      <QuoteSummary q={q} eyebrow="Your quote">
        <a className="btn btn-primary co-pay" href={url(`/buy?${configQuery(config)}`)}>
          Continue to checkout
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="square" aria-hidden="true"><path d="M5 12h14M13 6l6 6-6 6" /></svg>
        </a>
        <a className="btn btn-outline" href={mailto(`Quote: ${q.plan.name}, ${q.nodes} nodes`, quoteText(q).join('\n'))}>Send this quote to sales</a>
        <dl className="co-vs mono">
          <div><dt>{BASELINE.vendor} list, {q.nodes} hosts</dt><dd>{formatUsd(baseline)} / mo</dd></div>
          <div className="ours"><dt>This quote</dt><dd>{formatUsd(q.monthly)} / mo</dd></div>
          <p>{BASELINE.vendor}: {BASELINE.detail}.</p>
        </dl>
      </QuoteSummary>
    </div>
  );
}
